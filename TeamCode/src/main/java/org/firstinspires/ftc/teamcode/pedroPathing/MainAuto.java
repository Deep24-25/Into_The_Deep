...
public class MainAuto extends LinearOpMode {
    ...
    public static double PRELOAD_SCORE_X = 36.50;
    ...
    public static double PUSH_SAMPLES_INTERMEDIATARY_3_Y = 26.00;
    ...
    public static double SAMPLE_INTAKE_POS_X = 17;
    public static double SAMPLE_INTAKE_POS_Y = 24;
    ...
    public static double SLIDES_CAP_INTAKE = 0.3;
    ...
    public static boolean thirdSpecDevi = false;
    ...
    private final Pose sample2Pushed = new Pose(24, 12, Math.toRadians(180)); // x is 25 for 3 spec

    private final Pose sampleIntakePos = new Pose(SAMPLE_INTAKE_POS_X, SAMPLE_INTAKE_POS_Y, Math.toRadians(180));

    private final Pose sampleGripPos = new Pose(7, SAMPLE_INTAKE_POS_Y, Math.toRadians(180));

    private final Pose parkPose = new Pose(11, 18, Math.toRadians(180));
    ...
    // basket auto positions
    private final Pose basketStartPos = new Pose(7, 16, Math.toRadians(0));

    private final Pose basketDepositPos = new Pose(14, 56.5, Math.toRadians(-45));

    private final Pose basketIntakePos = new Pose(41.5, 52.5, Math.toRadians(0));

    private final Pose firstSampleIntakePos = new Pose(30, 48, Math.toRadians(0));


    private PathChain scorePreload, pushSamples, scoreFirstSpec, gripSpec, intakeSecondSpec, scoreSecondSpec, park;

    private PathChain basketIntake, preloadBasketDeposit, firstSampleIntakeBasket, secondSampleIntakeBasket, firstSampleIntake, secondSampleIntake, thirdSampleIntakeBasket, firstLevelHang, basketGoForward;
    ...
    public void buildPaths() {
      ...
      */
        ...
        gripSpec = follower.pathBuilder()
                .addPath(
                        new BezierLine(sampleIntakePos, sampleGripPos)
                )
                .build();
        scoreFirstSpec = follower.pathBuilder()
                .addPath(
                        // Line 1
                        new BezierCurve(
                                new Point(SAMPLE_INTAKE_POS_X, SAMPLE_INTAKE_POS_Y, Point.CARTESIAN),
                                new Point(2.000, 90.500, Point.CARTESIAN),
        ...
        basketIntake = follower.pathBuilder()
                .addPath(new BezierCurve(new Point(basketDepositPos), new Point(firstSampleIntakePos)))
                .setLinearHeadingInterpolation(basketDepositPos.getHeading(), firstSampleIntakePos.getHeading())
                .build();
        preloadBasketDeposit = follower.pathBuilder()
        ...
        firstLevelHang = follower.pathBuilder()
                .addPath(new BezierCurve(new Point(16, 56.5), new Point(60, 48), new Point(63, 24)))
                .setLinearHeadingInterpolation(-45, -90)
                .build();

        basketGoForward = follower.pathBuilder()
                .addPath(new BezierCurve(new Point(basketDepositPos), new Point(basketIntakePos)))
                .setLinearHeadingInterpolation(basketDepositPos.getHeading(), basketIntakePos.getHeading())
                .build();
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            try {
                ...
                logger.log("Loop time", timer.getElapsedTime(), Logger.LogLevels.PRODUCTION);
                ...
                logger.log("Deposit timer", depositTimer.getElapsedTimeSeconds(), Logger.LogLevels.PRODUCTION);
                monkeyPawFSM.log();
                limbFSM.log();
                logger.print();
            } catch (Exception e) {
    ...
    public void fourSpec() {
        ...
        switch (pathState) {
            ...
            case 2:
                ...
                break;
            case 3:
                if (limbFSM.DEPOSITED_SPECIMEN() || (depositTimer.getElapsedTimeSeconds() >= DEPOSIT_TIME)) {
                    monkeyPawFSM.setState(MonkeyPawFSM.States.DEPOSITING_SPECIMEN);
                    setPathState(4);
                }
                break;
            case 4:
                if (!follower.isBusy()) {
                    if (monkeyPawFSM.DEPOSITED_SPECIMEN() || limbFSM.PREPARING_TO_INTAKE_SPECIMEN()) {
                        if ((monkeyPawFSM.DEPOSITED_SPECIMEN() && (limbFSM.DEPOSITED_SPECIMEN() || (depositTimer.getElapsedTimeSeconds() >= 0))) || limbFSM.PREPARING_TO_INTAKE_SPECIMEN()) {
                            limbFSM.setStates(LimbFSM.States.PREPARING_TO_INTAKE_SPECIMEN);
                            monkeyPawFSM.setState(MonkeyPawFSM.States.PREPARING_TO_INTAKE_SPECIMEN);
                            setPathState(5);
                        }
            ...
            case 14:
                if (!follower.isBusy()) {
                    if (monkeyPawFSM.DEPOSITED_SPECIMEN() || limbFSM.PREPARING_TO_INTAKE_SPECIMEN()) {
                        if ((monkeyPawFSM.DEPOSITED_SPECIMEN() && (limbFSM.DEPOSITED_SPECIMEN() || (depositTimer.getElapsedTimeSeconds() >= DEPOSIT_TIME))) || limbFSM.PREPARING_TO_INTAKE_SPECIMEN()) {
                            limbFSM.setStates(LimbFSM.States.PREPARING_TO_INTAKE_SPECIMEN);
                            monkeyPawFSM.setState(MonkeyPawFSM.States.PREPARING_TO_INTAKE_SPECIMEN);
                            setPathState(15);
                        }
            ...
            case 20:
                ...
                break;
            case 21:
                if (limbFSM.EXTENDED_SPECIMEN() && monkeyPawFSM.READY_TO_DEPOSIT_SPECIMEN()) {
                    setPathState(22);
                }
                break;
            case 22:
                ...
                if (!follower.isBusy()) {
                    if (monkeyPawFSM.DEPOSITED_SPECIMEN() || limbFSM.PREPARING_TO_INTAKE_SPECIMEN()) {
                        if ((monkeyPawFSM.DEPOSITED_SPECIMEN() && (limbFSM.DEPOSITED_SPECIMEN() || (depositTimer.getElapsedTimeSeconds() >= DEPOSIT_TIME))) || limbFSM.PREPARING_TO_INTAKE_SPECIMEN()) {
                            limbFSM.setStates(LimbFSM.States.PREPARING_TO_INTAKE_SPECIMEN);
                            monkeyPawFSM.setState(MonkeyPawFSM.States.PREPARING_TO_INTAKE_SPECIMEN);
                            setPathState(35); // changed for park
                        }
                    }
                }
                break;
        ...
        switch (pathState) {
            ...
            case 4:
                if (monkeyPawFSM.PREPARED_TO_INTAKE_SAMPLE() && limbFSM.PREPARED_TO_INTAKE()) {
                    follower.followPath(basketGoForward, true);
                    /*if(!follower.isBusy()) {
    ...
    public void fourSample() {
        ...
        switch (pathState) {
            ...
            case 20:
                if (monkeyPawFSM.PREPARED_TO_INTAKE_SAMPLE() && limbFSM.PREPARED_TO_INTAKE()) {
                    follower.turn(Math.toRadians(70.0), true);
                    //follower.followPath(basketIntake);
                    if (!follower.isBusy()) {
                        setPathState(21);
                    }
            ...
            case 24:
                if (limbFSM.RETRACTED_FOR_AUTO()) {
                    follower.turn(Math.toRadians(70.0), false);
                    setPathState(25);
                }
                break;
            case 25:
            ...
            case 29:
                if (monkeyPawFSM.PREPARED_TO_INTAKE_SAMPLE() && limbFSM.PREPARED_TO_INTAKE()) {
                    follower.followPath(firstLevelHang, true);
                    limbFSM.setStates(LimbFSM.States.EXTENDING_SPECIMEN);
                    setPathState(30);
                }
                break;
        ...
        switch (pathState) {
            ...
            case 4:
                if (monkeyPawFSM.PREPARED_TO_INTAKE_SAMPLE() && limbFSM.PREPARED_TO_INTAKE()) {
                    ...
                    if (!follower.isBusy()) {
                        ...
                        follower.followPath(firstSampleIntakeBasket, true);
                        setPathState(5);
                    }
            ...
            case 12:
                if (monkeyPawFSM.PREPARED_TO_INTAKE_SAMPLE() && limbFSM.PREPARED_TO_INTAKE()) {
                    ...
                    if (!follower.isBusy()) {
                        follower.setMaxPower(0.5);
                        follower.followPath(secondSampleIntakeBasket, true);
                        setPathState(14);
                    }
                }
                break;
            ...
            case 20:
                if (monkeyPawFSM.PREPARED_TO_INTAKE_SAMPLE() && limbFSM.PREPARED_TO_INTAKE()) {
                    ...
                    if (!follower.isBusy()) {
                        ...
                        deviatorFSM.updatePos();
                        follower.setMaxPower(0.5);
                        follower.followPath(thirdSampleIntakeBasket, true);
                        setPathState(21);
                    }
                }
                break;
            case 21:
                if (!follower.isBusy() && deviatorFSM.atSetPoint()) {
                    monkeyPawFSM.setState(MonkeyPawFSM.States.INTAKING_SAMPLE);
                    setPathState(22);
                }
                break;
