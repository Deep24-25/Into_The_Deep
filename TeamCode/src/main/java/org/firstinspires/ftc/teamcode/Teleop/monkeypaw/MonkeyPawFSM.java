...
public class MonkeyPawFSM {
    ...
    public void findTargetState(boolean xPressed) {
        if ((limbFSM.PREPARED_TO_INTAKE() || limbFSM.PREPARING_TO_INTAKE() || limbFSM.MOVING_TO_INTAKE_POS() || limbFSM.EXTENDED_TO_DEPOSIT_SAMPLE()) && (!PREPARED_TO_INTAKE_SAMPLE() && !RELAXING_WITH_SAMPLE() && !RELAXED_POS_WITH_SAMPLE() && !RETRACTING_INTAKE())) {
        ...
        } else if (limbFSM.DEPOSITING_SPECIMEN() && READY_TO_DEPOSIT_SPECIMEN() && !DEPOSITED_SPECIMEN()) {
    ...
    public void updateState(boolean rightTrigger, boolean leftTrigger, boolean yPressed, boolean xPressed, boolean dpadUp, boolean dpadDown, boolean dpadRight, boolean dpadLeft, boolean dpadUp2, boolean dpadDown2, boolean isAuto) {
        ...
        switch (state) {
            ...
            case PREPARING_TO_INTAKE_SAMPLE:
                if (isAuto && !deviatorFSM.VERTICALED()) {
                    deviatorFSM.vertical();
                }
                if (rightTrigger) {
            ...
            case DEPOSITING_SPECIMEN:
                elbowFSM.flexToSpecimenLockPosition();
                if (elbowFSM.FLEXED_TO_CHAMBER_LOCK() && armFSM.AT_CHAMBER_LOCK_HEIGHT()) {
                    fingerFSM.releaseSpecimen();
                    if (fingerFSM.RELEASED()) {
